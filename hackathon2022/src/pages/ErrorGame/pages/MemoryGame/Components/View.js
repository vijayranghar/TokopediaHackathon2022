...
const PlayGround = () => {
  ...
  const { allImages, currentScore, currentLevel, energy, clicks, isCurrentLevelCompleted, time, levelFailed, restartGame, currentLevelsTrack, bestScore } = state;
  ...
  useEffect(() => {
    if (bestScore < currentScore) {
      dispatch({ type: ACTION_TYPES.DO_OPERATION, payload: { bestScore: currentScore } });
  ...
  useEffect(() => {
    ...
    dispatch({ type: ACTION_TYPES.DO_OPERATION, payload: { allImages: finalizedList, time: timeTrack, restartGame: false } });
    setTimeout(() => {
      const finalizedList = shuffledList.map((name, index) => {
        return ({
          name,
  ...
  useEffect(() => {
    if (time > 0 && !isCurrentLevelCompleted) {
      const interval = setTimeout(() => {
        dispatch({ type: ACTION_TYPES.DO_OPERATION, payload: { time: time - 1 } })
      }, 1000);
      return () => {
    ...
    if (openedImages.length === 2) {
      setTimeout(() => {
        ...
        if ((openedImages[0].name === openedImages[1].name) && (openedImages[0].index !== openedImages[1].index)) {
          ...
          dispatch({ type: ACTION_TYPES.DO_OPERATION, payload: { currentScore: currentScore + currentLevel, currentLevelsTrack: currentLevelsTrack + 1 } })
        } else {
          updatedImages[openedImages[0].index].close = true
          updatedImages[openedImages[1].index].close = true
        }
        setOpenedImage([]);
        dispatch({ type: ACTION_TYPES.DO_OPERATION, payload: { allImages: updatedImages,  ...(clickTrack && { clicks: clicks + 1 }) }});
      }, 750);
    } 
  }, [openedImages]);
...
const closeModal = () => {
  ...
  useEffect(() => {
    if (currentLevelsTrack && ((currentLevelsTrack/currentLevel) - (allImages.length/2) === 0)) {
      dispatch({ type: ACTION_TYPES.DO_OPERATION, payload: { isCurrentLevelCompleted: true, energy: energy + levels[currentLevel].bonus, currentLevelsTrack: 0 } })
    }
    if (currentLevelsTrack && (currentLevelsTrack % 10 === 0)) dispatch({ type: ACTION_TYPES.DO_OPERATION, payload: { energy: energy + 1 } })
  }, [currentLevelsTrack, allImages.length, currentLevel, energy]);

  // On level completion
  useEffect(() => {
    if (isCurrentLevelCompleted && !isModalOpen)
      setModal(true);
  ...
  useEffect(() => {
    if (clicks === clickTrack && !isCurrentLevelCompleted) {
      dispatch({ type: ACTION_TYPES.DO_OPERATION, payload: { levelFailed: 'click exceed' } });
    }
    if (time === 0 && !isCurrentLevelCompleted) {
      dispatch({ type: ACTION_TYPES.DO_OPERATION, payload: { levelFailed: 'timeout' } });
    }
    ...
    return (
      <div className='container'>
        ...
        <section className='user-score-data'>
          <div className='box'>
          <p>Level: {currentLevel}/{totalLevelsInGame}</p>
          <p>Energy: {energy}</p>
          </div>
          <div className='box'>
            {timeTrack ? <p>Time {convertSecondsToTime(time)}</p> : null}
            {clickTrack ? <p>Flips: {clicks} / {clickTrack}</p> : null}
          </div>
          <div className='box'>
        ...
        </div>
        {
          isCurrentLevelCompleted ? 
            <Modal isOpen={isModalOpen} onClose={closeModal}>
              <LevelComplete dispatch={dispatch} currentLevel={currentLevel} clicks={clicks} time={timeTrack - time} closeModal={closeModal} />
            </Modal> 
            : 
            null
        }
        {
          levelFailed ?
            <Modal>
              <LevelFailed currentLevel={currentLevel} dispatch={dispatch} clicks={clicks} time={timeTrack - time} levelFailed={levelFailed} closeModal={closeModal} />
            </Modal>
            :
            null
        }
